...
public class SearchControlServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String skinname=request.getParameter("id");
        SkinDAO skindao= new SkinDAO();
        int pagenum=skindao.count(skinname);
        int pagesize=12;
        ...
        int x=0;
        int y=0;
        List<Skin> list =skindao.search(skinname,pageindex,pagesize);
        request.setAttribute("pageindex", pageindex);
        request.setAttribute("list",list);
        request.setAttribute("pagesize", pagesize);
        request.setAttribute("end", endpage);
        request.setAttribute("id", skinname);
        request.setAttribute("url", "SearchControlServlet?pageindex=");
