...
public class ShowStock extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int pagenum = skindao.userStockCount(id);
            int pagesize = 12;
            int endpage = pagenum / pagesize;
            if (pagenum % pagesize != 0) {
                endpage++;
            }
            List<Skin> list = skindao.getUserStock(id, pageindex, pagesize);
            request.setAttribute("pageindex", pageindex);
            request.setAttribute("list", list);
            request.setAttribute("pagesize", pagesize);
            request.setAttribute("end", endpage);
            request.setAttribute("url", "ShowStock?pageindex=");
            int x = 0;
            int y = 0;
            ...
            request.getRequestDispatcher("yourstock.jsp").forward(request, response);
        } catch (Exception e) {
